...
class StudentRecords(QWidget):
    ...
    def delete_student(self):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            from dal import db_dal
            try:
                success = db_dal.delete_student(int(self.selected_student_id))
                if success:
        ...
        def save_changes():
            ...
            try:
                success = db_dal.update_student(
                    int(self.selected_student_id),
                    name_entry.text().strip(),
                    class_entry.text().strip(),
                    int(parent_entry.text().strip()) if parent_entry.text().strip().isdigit() else None,
                    int(route_entry.text().strip()) if route_entry.text().strip().isdigit() else None,
                    fee_paid,
                    fee_balance
                )
                if success:
